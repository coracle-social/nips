...
- `public` signals that messages within the group can be _read_ by anyone, while `private` signals that only members can read messages.
- `open` signals that anyone can post to the group, while `closed` signals that only members can post to the group.
