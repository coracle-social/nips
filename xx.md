...
A `kind 30301` event indicates room membership. If absent, then everyone is considered a member. Clients should request this event in order to determine the user's membership status. The membership event's author MUST be the same as the room metadata's author.
